...
const ProjectsPage = () => {
    ...
    const { data: projects, error } = useGetProjectsDetailsQuery();

    const token = useSelector((state: RootState) => {state.user.token});
    const { data: user } = useGetUserDetailsQuery(token, {
        ...
        <> 
            <EthosBody nav>
                ...
                {user && (
                    ...
                    <section>
                        <div className="mx-1 max-w-screen-xl py-8 sm:py-12 md:mx-8">
                            ...
                            { projects?.map((project, index) => (
                                ...
                                <li key={index} className="flex transition hover:shadow-2xl shadow-lg w-[84vw] hover:translate-x-1 hover:translate-y-2 sm:w-[85vw] mx-auto lg:w-[80vw] sm:mx-auto sm:px-1 mt-[2%] md:py-2 glass flex-col py-2 text-left sm:flex-row sm:space-x-5 rounded-md p-4 sm:flex sm:justify-start glass-effect my-[5%]">
                                    <div className="rotate-180 p-2 [writing-mode:_vertical-lr]">
                                        <div
                                        className="flex items-center justify-between gap-4 text-xs font-bold uppercase text-gray-900"
                                        >
                                        <span className="w-px flex-1 bg-gray-900/10 dark:bg-color-3"></span>
                                        <span className="hidden md:block"><FolderOpenIcon className="h-5 w-5 rotate-180 dark:text-color-3" /></span>
                                        </div>
                                        <span className="flex relative sm:my-[4px] mx-auto font-iceberg dark:text-color-3"> [0{index + 1}]</span>
                                    </div>

                                    {/* <div className="sm:block basis-56 sm:basis-36"> */}
                                    <div className="relative shrink-0 my-auto hover:scale-105">

                                        <img
                                        alt=""
                                        src={`${Config.baseURL}/static/images/project_images/${project.images[0]}`}
                                        className="aspect-square h-[100px] w-full object-cover transition duration-500 group-hover:scale-105 md:h-[8rem]"
                                        />
                                    </div>
                                    {user && (
                                        <>
                                            <button onClick={() => handleDelete(project._id)}>
                                            <span className="m-1 p-2 bg-red-600 text-black dark:text-white">Delete Project</span>
                                            </button>
                                        </>
                                    )}
                                    <div className="flex flex-1 flex-col justify-between md:my-auto w-fit">
                                        <div className="border-s border-gray-900/10 p-4 sm:border-l-transparent sm:p-[1em]">
                                        <a href="#">
                                            <h3 className="font-bold tracking-wider font-iceberg capitalize text-n-4 dark:text-color-7 sm:text-3xl ">
                                            {project.title}
                                            </h3>
                                        </a>

                                        <p className="mt-1 line-clamp-3 text-sm/relaxed text-n-6 dark:text-color-7">
                                            {project.description}
                                        </p>
                                        </div>

                                        <div className="flex items-end gap-2 m-1 sm:justify-end">
                                            {project.github_url && (
                                                <>
                                                    <a
                                                        href={`${project.github_url}`}
                                                        className="block bg-color-1 px-2 py-2 text-center text-xs font-bold uppercase text-gray-900 transition hover:bg-yellow-400 rounded-lg"
                                                    >
