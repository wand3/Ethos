...
export const NavMain = () => {
  ...
  return (
    <div
      className={`fixed lg:w-[96vw] top-[2vh] left-1/2 w-[96%]  -translate-x-1/2 z-50 glass-effect backdrop-blur-lg lg:backdrop-blur-lg drop-shadow-md dark:text-white ${
        openNavigation ? "h-screen" : "backdrop-blur-sm"
      }`}
    >
      <div className="flex items-center px-2 lg:px-7.5 xl:px-10 max-lg:py-4">
        <div className="flex w-[30%] h-10 opacity-100 right-[70%] float-end ml-[3vw]">
          <a className="flex xl:mr-8 mr-2" href="#hero">
            <img src={w} className='w-[3rem] flex' alt="Wande" />
            {/* <span className='text-2xl font-bold flex relative items-center'>Wa<span className='text-white px-1 dark:text-black'>nde</span>
              <div className='bg-black dark:bg-[#ffffff] w-[52%] h-[1.5em] left-[45%] -z-[2] absolute'>
          ...
          <div className="relative z-2 flex flex-col items-center justify-center lg:flex-row m-auto">
            {navigation.map((item) => (
              <a
                ...
                href={item.url}
                onClick={handleClick}
                className={`block relative font-code text-lg capitalize text-n-1 dark:text-white transition-colors hover:shadow-lg dark:hover:text-color-1 border-b-[transparent] dark:shadow-2xl lg:flex-row ${
                  item.onlyMobile ? "lg:hidden" : ""
                } px-6 py-6 md:py-8 lg:-mr-0.25 lg:text-xs lg:font-semibold ${
                  item.url === pathname.hash
                    ? "z-2 lg:text-n-3"
